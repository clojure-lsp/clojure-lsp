...
(defn- swap-namespace-with-alias [code ns new-alias]
  (f.add-missing-libspec/swap-namespace-with-alias (h/zloc-from-code code) ns new-alias))

(deftest swap-namespace-with-alias-test
  (testing "simple substitution"
    (is (= ["[my.ns.xyz :as xyz]"
            "xyz/my-func"]
           (as-strs (swap-namespace-with-alias (h/code "(ns foo (:require [my.ns.xyz])) |my.ns.xyz/my-func") "my.ns.xyz" "xyz")))))
  (testing "verify ranges"
    (is (= [{:col 19, :end-col 30, :end-row 1, :row 1}
            {:col 33, :end-col 50, :end-row 1, :row 1}]
           (map :range (swap-namespace-with-alias (h/code "(ns foo (:require [my.ns.xyz])) |my.ns.xyz/my-func") "my.ns.xyz" "xyz")))))
  (testing "substitution with libspec not in vector"
    (is (= ["[my.ns.xyz :as xyz]"
            "xyz/my-func"]
           (as-strs (swap-namespace-with-alias (h/code "(ns foo (:require my.ns.xyz)) |my.ns.xyz/my-func") "my.ns.xyz" "xyz")))))
  (testing "substitution with incomplete require"
    (is (nil? (swap-namespace-with-alias (h/code "(ns foo (:require)) |my.ns.xyz/my-func") "my.ns.xyz" "xyz"))))
  (testing "substitution with missing libspec"
    (is (nil? (swap-namespace-with-alias (h/code "(ns foo (:require [clojure.string])) |my.ns.xyz/my-func") "my.ns.xyz" "xyz"))))
  (testing "substitution with libspec that is missing actual alias (is incomplete)"
    (is (= ["[my.ns.xyz :as xyz]"
            "xyz/my-func"]
           (as-strs (swap-namespace-with-alias (h/code "(ns foo (:require [my.ns.xyz :as])) |my.ns.xyz/my-func") "my.ns.xyz" "xyz")))))
  (testing "substitution with libspec that has a refer but no alias"
    (is (= ["[my.ns.xyz :as xyz :refer [abc]]"
            "xyz/my-func"]
           (as-strs (swap-namespace-with-alias (h/code "(ns foo (:require [my.ns.xyz :refer [abc]])) |my.ns.xyz/my-func") "my.ns.xyz" "xyz")))))
  (testing "substitution with libspec that has an alias but also an out of order refer"
    (is (= ["xyz/my-func"]
           (as-strs (swap-namespace-with-alias (h/code "(ns foo (:require [my.ns.xyz :refer [abc] :as xyz])) |my.ns.xyz/my-func") "my.ns.xyz" "xyz")))))
  (testing "require has :import as well as :require"
    (is (= ["[my.ns.xyz :as xyz]" "xyz/my-func"]
           (as-strs (swap-namespace-with-alias (h/code "(ns foo (:import [java.util AbstractList]) (:require [my.ns.xyz])) |my.ns.xyz/my-func")
                                               "my.ns.xyz" "xyz")))))
  (testing "require has :import as well as :require, check locations"
    (is (= [{:col 54, :end-col 65, :end-row 1, :row 1} {:col 68, :end-col 85, :end-row 1, :row 1}]
           (map :range (swap-namespace-with-alias (h/code "(ns foo (:import [java.util AbstractList]) (:require [my.ns.xyz])) |my.ns.xyz/my-func")
                                                  "my.ns.xyz" "xyz")))))
  (testing "verify ranges when replacing libspec with libspec in a vector"
    (is (= [{:col 31, :end-col 40, :end-row 1, :row 1} {:col 43, :end-col 60, :end-row 1, :row 1}]
           (map :range (swap-namespace-with-alias (h/code "(ns foo (:require [my.ns.xyz] my.ns.abc)) |my.ns.xyz/my-func") "my.ns.abc" "abc")))))
  (testing "when alias already exists, don't add it again"
    (is (= ["xyz/my-func"]
           (as-strs (swap-namespace-with-alias (h/code "(ns foo (:require [my.ns.xyz :as xyz])) |my.ns.xyz/my-func") "my.ns.xyz" "xyz"))))))
