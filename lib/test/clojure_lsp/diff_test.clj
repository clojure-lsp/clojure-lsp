...
(deftest ->chunks-test
  (let [mkdocs-diff ["diff --git a/mkdocs.yml b/mkdocs.yml"
        ...
        mkdocs-expected {:file "mkdocs.yml"
                         :added-line-numbers []}
        internal-api-diff ["diff --git a/lib/src/clojure_lsp/internal_api.clj b/lib/src/clojure_lsp/internal_api.clj"
    ...
    (testing "Given a diff output with two files
      ...
      (is (match? [mkdocs-expected internal-api-expected]
                  (diff/->chunks
                    (string/join "\n"
                                 (concat mkdocs-diff internal-api-diff))))))))
