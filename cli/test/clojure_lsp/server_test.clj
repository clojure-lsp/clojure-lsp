...
(deftest test-client-settings
  (testing "initializationOptions are null"
    (let [params (InitializeParams.)]
      ...
      (is (= "zipfile"
             (get (#'lsp/client-settings params) :dependency-scheme)))))
  (testing "document-formatting? is set to true if not provided"
    (let [params (InitializeParams.)]
      (is (= true (:document-formatting? (#'lsp/client-settings params))))))
  (testing "document-range-formatting? is set to true if not provided"
    (let [params (InitializeParams.)]
      (is (= true (:document-range-formatting? (#'lsp/client-settings params))))))
  (testing "text-document-sync-kind converts strings"
    (doseq [kind [:incremental "incremental" ":incremental"]]
      (let [params (doto (InitializeParams.)
        ...
        (is (= :incremental (:text-document-sync-kind (#'lsp/client-settings params)))))))
    ...
    (doseq [path [["foo"] [":foo"]]]
      (let [params (doto (InitializeParams.)
                     (.setInitializationOptions
                       (coercer/clj->java
                         {:source-paths path})))]
        (is (= #{"foo"} (:source-paths (#'lsp/client-settings params)))))))
    ...
    (doseq [path [[:foo] ["foo"] [":foo"]]]
      (let [params (doto (InitializeParams.)
                     (.setInitializationOptions
                       (coercer/clj->java
                         {:source-aliases path})))]
        (is (= #{:foo} (:source-aliases (#'lsp/client-settings params)))))))
  (testing "vectors and keywords are converted properly"
    (let [params (doto (InitializeParams.)
                   (.setInitializationOptions
                     (coercer/clj->java
                       {:cljfmt {:indents {'something [[:block 1]]}}})))]
      (is (= {:indents {'something [[:block 1]]}} (:cljfmt (#'lsp/client-settings params)))))))
