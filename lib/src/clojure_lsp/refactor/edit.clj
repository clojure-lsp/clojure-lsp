...
(defn ^:private in-range-or-incomplete?
  "Checks whether the `loc`s node is [[in-range?]] of the given `pos`, or if it
  is some incomplete leaf-node code like `foo/`, which the parser didn't give a
  position."
  [loc pos]
...
(defn find-by-heritability
  "Find the deepest, right-most zloc from `start-zloc` that satisfies
  `inherits?`. `inherits?` must be a function such that if zloc satisifies it
  then so will all of its ancestors. If a parent node satisifies `inherits?` but
  none of its children do, then this returns the parent, on the assumption that
  the parent is the last in its lineage with the trait.
...
(defn find-at-pos
  "Find the deepest, right-most zloc whose node is at the given `row` and `col`,
  seeking from initial zipper location `zloc`.

  This is similar to z/find-last-by-pos, but is faster, permits incomplete nodes
  generated by the parser, and doesn't require {:track-position? true}."
  [zloc row col]
  (let [exact-position {:row row, :col col, :end-row row, :end-col col}]
    (find-by-heritability zloc #(in-range-or-incomplete? % exact-position))))
