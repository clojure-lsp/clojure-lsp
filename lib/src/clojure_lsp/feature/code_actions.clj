...
(defn ^:private extract-function-2-action [uri start-line start-character end-line end-character]
  {:title   "Extract function 2"
   :kind    :refactor-extract
   :command {:title     "Extract function 2"
             :command   "extract-function-2"
             :arguments [uri start-line start-character end-line end-character "new-function-2"]}})
...
;; the original all function didn't include parameters for end-row and end-col in a 
;; selection, now the seven argument function is included so the tests don't need to add nils
(defn all
   ...
   (let [zloc (parser/to-pos root-zloc row col)
     ...
     (cond-> []
       ...
       (conj (move-to-let-action uri line character)
             (extract-function-2-action uri line character end-line end-character))
