(ns clojure-lsp.server
  (:require
   ...
   [jsonrpc4clj.coercer :as jsonrpc.coercer]
   [jsonrpc4clj.io-server :as jsonrpc.io-server]
   [jsonrpc4clj.liveness-probe :as jsonrpc.liveness-probe]
...
(defmacro conform-or-log
  ...
  (let [fmeta (assoc (meta &form)
                     :file *file*
                     :ns-str (str *ns*))]
    `(jsonrpc.coercer/conform-or-log
       (fn [& args#]
    ...
    (jsonrpc.server/discarding-stdout
      (shared/logging-task
        ...
        (some->> diagnostic
                 ...
                 (jsonrpc.server/send-notification server "textDocument/publishDiagnostics")))))

  (refresh-code-lens [_this]
    (jsonrpc.server/discarding-stdout
      (when (get-in @db* [:client-capabilities :workspace :code-lens :refresh-support])
        (jsonrpc.server/send-request server "workspace/codeLens/refresh" nil))))

  (publish-workspace-edit [_this edit]
    (jsonrpc.server/discarding-stdout
      (when-let [conformed (conform-or-log ::coercer/workspace-edit-params {:edit edit})]
        (let [request (jsonrpc.server/send-request server "workspace/applyEdit" conformed)
  ...
  (publish-progress [_this percentage message progress-token]
    (jsonrpc.server/discarding-stdout
      ...
      (->> (lsp.requests/work-done-progress percentage message progress-token)
           (jsonrpc.server/send-notification server "$/progress"))))
    ...
    (jsonrpc.server/discarding-stdout
      (when-let [conformed (conform-or-log ::coercer/show-message-request
                                           {:type    type
                                            :message message
                                            :actions actions})]
        (let [request (jsonrpc.server/send-request server "window/showMessageRequest" conformed)
              ;; High timeout as we probably want to wait some time for user input
              response (jsonrpc.server/deref-or-cancel request 10e5 ::timeout)]
      ...
      (let [message-content {:message message
        ...
        (shared/logging-task
          ...
          (some->> message-content
                   ...
                   (jsonrpc.server/send-notification server "window/showMessage"))))))
            ...
            (shared/logging-task
              ...
              (doseq [uri uris]
                (some->> (f.test-tree/tree uri db)
                         ...
                         (jsonrpc.server/send-notification server "clojure/textDocument/testTree"))))))))))

;;;; clojure extra features

(defmethod jsonrpc.server/receive-request "clojure/dependencyContents" [_ components params]
  (->> (medley/update-existing params :uri normalize-uri)
       (handler/dependency-contents components)
       (conform-or-log ::coercer/uri)
       eventually))

(defmethod jsonrpc.server/receive-request "clojure/serverInfo/raw" [_ components _params]
  (->> components
       handler/server-info-raw
       eventually))

(defmethod jsonrpc.server/receive-notification "clojure/serverInfo/log" [_ components _params]
  (future
    (try
...
(defmethod jsonrpc.server/receive-request "clojure/cursorInfo/raw" [_ components params]
  (->> params
       normalize-doc-uri
       (handler/cursor-info-raw components)
       eventually))

(defmethod jsonrpc.server/receive-notification "clojure/cursorInfo/log" [_ components params]
  (future
    (try
...
(defmethod jsonrpc.server/receive-request "clojure/clojuredocs/raw" [_ components params]
  (->> params
       (handler/clojuredocs-raw components)
       eventually))

(defmethod jsonrpc.server/receive-request "clojure/workspace/projectTree/nodes" [_ components params]
  (->> params
...
(defmethod jsonrpc.server/receive-notification "textDocument/didOpen" [_ components params]
  (handler/did-open components (normalize-doc-uri params)))

(defmethod jsonrpc.server/receive-notification "textDocument/didChange" [_ components params]
  (handler/did-change components (normalize-doc-uri params)))

(defmethod jsonrpc.server/receive-notification "textDocument/didSave" [_ components params]
  (future
    (try
...
(defmethod jsonrpc.server/receive-notification "textDocument/didClose" [_ components params]
  (handler/did-close components (normalize-doc-uri params)))

(defmethod jsonrpc.server/receive-request "textDocument/references" [_ components params]
  (->> params
       normalize-doc-uri
...
(defmethod jsonrpc.server/receive-request "textDocument/completion" [_ components params]
  (->> params
       normalize-doc-uri
...
(defmethod jsonrpc.server/receive-request "completionItem/resolve" [_ components item]
  (->> item
...
(defmethod jsonrpc.server/receive-request "textDocument/prepareRename" [_ components params]
  (->> params
       normalize-doc-uri
...
(defmethod jsonrpc.server/receive-request "textDocument/rename" [_ components params]
  (->> params
       normalize-doc-uri
...
(defmethod jsonrpc.server/receive-request "textDocument/hover" [_ components params]
  (->> params
       normalize-doc-uri
...
(defmethod jsonrpc.server/receive-request "textDocument/signatureHelp" [_ components params]
  (->> params
       normalize-doc-uri
...
(defmethod jsonrpc.server/receive-request "textDocument/rangeFormatting" [_this components params]
  (->> params
       normalize-doc-uri
...
(defmethod jsonrpc.server/receive-request "textDocument/codeAction" [_ components params]
  (->> params
       normalize-doc-uri
...
(defmethod jsonrpc.server/receive-request "textDocument/codeLens" [_ components params]
  (->> params
       normalize-doc-uri
...
(defmethod jsonrpc.server/receive-request "codeLens/resolve" [_ components code-lens]
  (->> code-lens
       (handler/code-lens-resolve components)
       (conform-or-log ::coercer/code-lens-or-error)
       eventually))

(defmethod jsonrpc.server/receive-request "textDocument/definition" [_ components params]
  (->> params
       normalize-doc-uri
...
(defmethod jsonrpc.server/receive-request "textDocument/declaration" [_ components params]
  (->> params
       normalize-doc-uri
...
(defmethod jsonrpc.server/receive-request "textDocument/implementation" [_ components params]
  (->> params
       normalize-doc-uri
...
(defmethod jsonrpc.server/receive-request "textDocument/documentSymbol" [_ components params]
  (->> params
       normalize-doc-uri
...
(defmethod jsonrpc.server/receive-request "textDocument/documentHighlight" [_ components params]
  (->> params
       normalize-doc-uri
...
(defmethod jsonrpc.server/receive-request "textDocument/semanticTokens/full" [_ components params]
  (->> params
       normalize-doc-uri
...
(defmethod jsonrpc.server/receive-request "textDocument/semanticTokens/range" [_ components params]
  (->> params
       normalize-doc-uri
...
(defmethod jsonrpc.server/receive-request "textDocument/prepareCallHierarchy" [_ components params]
  (->> params
       normalize-doc-uri
...
(defmethod jsonrpc.server/receive-request "callHierarchy/incomingCalls" [_ components params]
  (->> params
       (handler/call-hierarchy-incoming components)
       (conform-or-log ::coercer/call-hierarchy-incoming-calls-or-error)
       eventually))

(defmethod jsonrpc.server/receive-request "callHierarchy/outgoingCalls" [_ components params]
  (->> params
       (handler/call-hierarchy-outgoing components)
       (conform-or-log ::coercer/call-hierarchy-outgoing-calls-or-error)
       eventually))

(defmethod jsonrpc.server/receive-request "textDocument/linkedEditingRange" [_ components params]
  (->> params
       normalize-doc-uri
...
(defmethod jsonrpc.server/receive-request "textDocument/foldingRange" [_ components params]
  (->> params
       normalize-doc-uri
...
(defmethod jsonrpc.server/receive-request "textDocument/selectionRange" [_ components params]
  (->> params
       normalize-doc-uri
...
(defmethod jsonrpc.server/receive-request "workspace/executeCommand" [_ components params]
  (->> params
       (handler/execute-command components)
       (conform-or-log ::coercer/any-or-error)
       eventually))

(defmethod jsonrpc.server/receive-notification "workspace/didChangeConfiguration" [_ components params]
  (->> params
       (handler/did-change-configuration components)
       eventually))

(defmethod jsonrpc.server/receive-notification "workspace/didChangeWatchedFiles" [_ components params]
...
(defmethod jsonrpc.server/receive-request "workspace/symbol" [_ components params]
  (->> params
       (handler/workspace-symbols components)
       (conform-or-log ::coercer/workspace-symbols-or-error)
       eventually))

(defmethod jsonrpc.server/receive-request "workspace/willRenameFiles" [_ components params]
  (->> (medley/update-existing params
                               :files (fn [files]
                                        (mapv (fn [file]
                                                (-> file
...
(defmethod jsonrpc.server/receive-notification "workspace/didRenameFiles" [_ components params]
  (->> params
...
(defmethod jsonrpc.server/receive-request "initialize" [_ {:keys [db* server] :as components} params]
  ...
  (when-let [trace-level (:trace params)]
    (jsonrpc.server/set-trace-level server trace-level))
  (when-let [parent-process-id (:process-id params)]
    (jsonrpc.liveness-probe/start! parent-process-id log-wrapper-fn #(exit server)))
  {:capabilities (capabilities (settings/all @db*))})

(defmethod jsonrpc.server/receive-notification "initialized" [_ {:keys [server] :as components} _params]
  (when-let [register-capability (handler/initialized components known-files-pattern)]
    (jsonrpc.server/send-request server "client/registerCapability" register-capability)))
...
(defmethod jsonrpc.server/receive-notification "$/setTrace" [_ {:keys [server]} {:keys [value]}]
  (shared/logging-task
    :set-trace
    (jsonrpc.server/set-trace-level server value)))
...
(defn run-lsp-io-server! [trace-level log-path]
  (jsonrpc.server/discarding-stdout
    (let [log-ch (async/chan (async/sliding-buffer 20))
          server (jsonrpc.io-server/stdio-server {:log-ch log-ch
                                                  :trace-ch log-ch
