        ...
        findings (->> (clj-kondo.core/run! {:lint [filename]})
                      ...
                      (mapv (fn [finding]
                              (#'clojure-lsp.feature.diagnostics/kondo-finding->diagnostic
                               ...
                               nil
                               finding))))]
    (let [[old-loc new-zloc] (mock-symbols* zloc findings)]
      (r.transform/locs-to-ranges
        [{:range (meta (z/node old-loc))
          :loc new-zloc}]))
    #_(clojure-lsp.refactor.transform/locs-to-ranges (mock-symbols* zloc findings))
    #_(print-loc
...
;; TODO I think clj-kondo has this already
