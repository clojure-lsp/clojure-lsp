...
(deftest find-full-macro-symbol-to-resolve
  ...
  (testing "inside a macro usage"
    (is (= 'some-ns/foo
           (f.resolve-macro/find-full-macro-symbol-to-resolve (h/file-uri "file:///a.clj") 3 7 db/db))))
  (testing "not inside a macro usage"
    (is (not (f.resolve-macro/find-full-macro-symbol-to-resolve (h/file-uri "file:///a.clj") 4 5 db/db)))))
...
(deftest resolve-macro-as
  ...
  (testing "resolving macro as def"
    (is (= "{:lint-as {some-ns/foo clojure.core/def}}\n"
           (#'f.resolve-macro/resolve-macro-as (h/file-uri "file:///a.clj") 3 7 "clojure.core/def" ".any-clj-kondo/config.edn" db/db)))))
