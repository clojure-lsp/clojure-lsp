...
| `:hover` `:hide-signature-call?`               | Whether to show the surrounding call on hover.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | `false`                                                                                       |
| `:source-paths-ignore-regex`                   | list of regex to filter source-paths. By default, source-paths are retrieved from classpath , and usually the classpath contains folders that are not directly project code or it's auto-generated like cljs `resources` or `target` folders but it's inside your project. Replace old `:ignore-classpath-directories` setting. This operates on relative paths.                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | `["target.*"]`                                                                                |
| `:paths-ignore-regex`                          | list of regex to filter paths to be analyzed. Useful for excluding huge files or folders that may affect final analysis size and performance, like huge edn files. These are absolute paths, so make sure that regex pattern matches fully expanded path.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | `[]`                                                                                          |
...
| `:private-by-default-on-extract?`              | Whether extracted functions and defs should be private by default. When `false`, extract function/def will produce public `defn`/`def` instead of `defn-`/`def ^:private`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | `true`                                                                                        |
...
| `:semantic-tokens` `:default-library-for-core?` | Whether to add the `defaultLibrary` semantic token modifier to `clojure.core`/`cljs.core` functions and macros, allowing editors to highlight them distinctly.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `false`                                                                                       |
...
| `:test-locations-regex`                        | A set of patterns (regex strings) specifying where the tests live. Patterns are matched with [clojure.core/re-find](https://clojuredocs.org/clojure.core/re-find) against file URI strings. | `#{"_test\.clj[a-z]?$"}` |
...
| `:analysis` `:java` `:member-definitions`      | How java member definitions are analyzed. `:lazy`/`:on-demand` analyzes a class's members the first time it is navigated/hovered/completed (lowest memory); `true` analyzes every dependency's members up front (eager); `false` disables it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | `:lazy`                                                                                       |
| `:analysis` `:symbols`                         | Whether to enable symbols analysis for performance.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | `true`                                                                                        |
| `:otlp`                             | [OpenTelemetry](https://opentelemetry.io/) related settings, check the [Opentelemetry](#opentelemetry-integration) section below.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | `false`                                                                                       |
...
- `squint`: If a `squint.edn` is found at the project root, clojure-lsp will run `clojure -Sdeps squint.edn -Spath` with `:source-aliases` specified if any. It's recommended to configure inside `squint.edn`, in your source-alias (`dev` for example), the clojurescript dependency. Example: `:aliases {:dev {:extra-deps {org.clojure/clojurescript {:mvn/version "1.12.42"}}}}`
...
- `:ignore-test-references?` whether to ignore test references of a var-definition when considering if the var is unused. Useful to check functions that have tests but are not being used in the source code. Relies on Clojure-LSP's `:test-locations-regex` setting to locate test files. Default: `false`.
