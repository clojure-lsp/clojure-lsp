...
(deftest internal-analysis
  (testing "when dependency-scheme is zip"
    (let [components (h/make-components)]
      (h/load-code "(ns foo.bar)" (h/file-uri "file:///a.clj") components)
      (h/load-code "(ns foo.bar)" (h/file-uri "file:///b.clj") components)
      (h/load-code "(ns foo.bar)" (h/file-uri "jar:file:///some.jar!/some-file.clj") components)
      (is (= 2 (count (q/internal-analysis (h/db components)))))))
  (testing "when dependency-scheme is jar"
    (let [components (h/make-components {:settings {:dependency-scheme "jar"}})]
      (h/load-code "(ns foo.bar)" (h/file-uri "file:///a.clj") components)
      (h/load-code "(ns foo.bar)" (h/file-uri "file:///b.clj") components)
      (h/load-code "(ns foo.bar)" (h/file-uri "jar:file:///some.jar!/some-file.clj") components)
      (is (= 2 (count (q/internal-analysis (h/db components))))))))
...
(deftest external-analysis
  (testing "when dependency-scheme is zip"
    (let [components (h/make-components)]
      ...
      (h/load-code "(ns foo.bar)" (h/file-uri "file:///b.clj") components)
      (h/load-code "(ns foo.bar)" (h/file-uri "jar:file:///some.jar!/some-file.clj") components)
      (is (= 1 (count (q/external-analysis (h/db components)))))))
  (testing "when dependency-scheme is jar"
    (let [components (h/make-components {:settings {:dependency-scheme "jar"}})]
      (h/load-code "(ns foo.bar)" (h/file-uri "file:///a.clj") components)
      (h/load-code "(ns foo.bar)" (h/file-uri "file:///b.clj") components)
...
(deftest ns-analysis
  (let [components (h/make-components)]
    (h/load-code "(ns foo.bar)" (h/file-uri "file:///a.clj") components)
    (h/load-code "(ns foo.bar)" (h/file-uri "file:///b.clj") components)
    (h/load-code "(ns foo.baz)" (h/file-uri "file:///c.clj") components)
    (is (= 2 (count (q/ns-analysis (h/db components) 'foo.bar))))))

(deftest ns-dependents-analysis
  (let [components (h/make-components)]
    (h/load-code "(ns aaa (:require [bbb] [ccc]))" (h/file-uri "file:///aaa.clj") components)
    (h/load-code "(ns bbb (:require [ccc]))" (h/file-uri "file:///bbb.clj") components)
    (h/load-code "(ns ccc)" (h/file-uri "file:///ccc.clj") components)
    (is (= 0 (count (q/ns-dependents-analysis (h/db components) 'aaa))))
    (is (= 1 (count (q/ns-dependents-analysis (h/db components) 'bbb))))
    (is (= 2 (count (q/ns-dependents-analysis (h/db components) 'ccc))))))
...
(deftest ns-and-dependents-analysis
  (let [components (h/make-components)]
    ...
    (h/load-code "(ns bbb (:require [ccc]))" (h/file-uri "file:///bbb.clj") components)
    (h/load-code "(ns ccc)" (h/file-uri "file:///ccc.clj") components)
    (is (= 1 (count (q/ns-and-dependents-analysis (h/db components) 'aaa))))
    (is (= 2 (count (q/ns-and-dependents-analysis (h/db components) 'bbb))))
    (is (= 3 (count (q/ns-and-dependents-analysis (h/db components) 'ccc))))))
...
(deftest ns-dependencies-analysis
  (let [components (h/make-components)]
    ...
    (h/load-code "(ns bbb (:require [ccc]))" (h/file-uri "file:///bbb.clj") components)
    (h/load-code "(ns ccc)" (h/file-uri "file:///ccc.clj") components)
    (is (= 2 (count (q/ns-dependencies-analysis (h/db components) 'aaa))))
    (is (= 1 (count (q/ns-dependencies-analysis (h/db components) 'bbb))))
    (is (= 0 (count (q/ns-dependencies-analysis (h/db components) 'ccc))))))
...
(deftest nses-analysis
  (let [components (h/make-components)]
    ...
    (h/load-code "(ns ccc)" (h/file-uri "file:///ccc.clj") components)
    (is (= 1 (count (q/nses-analysis (h/db components) '#{aaa}))))
    (is (= 2 (count (q/nses-analysis (h/db components) '#{aaa bbb}))))
    (is (= 3 (count (q/nses-analysis (h/db components) '#{aaa bbb ccc}))))))
...
(deftest nses-and-dependents-analysis
  (let [components (h/make-components)]
    ...
    (h/load-code "(ns bbb (:require [ccc]))" (h/file-uri "file:///bbb.clj") components)
    (h/load-code "(ns ccc)" (h/file-uri "file:///ccc.clj") components)
    (is (= 1 (count (q/nses-and-dependents-analysis (h/db components) '#{aaa}))))
    (is (= 2 (count (q/nses-and-dependents-analysis (h/db components) '#{bbb}))))
    (is (= 3 (count (q/nses-and-dependents-analysis (h/db components) '#{ccc}))))
    (is (= 2 (count (q/nses-and-dependents-analysis (h/db components) '#{aaa bbb}))))))
...
(deftest uri-dependents-analysis
  (let [components (h/make-components)]
    ...
    (h/load-code "(ns bbb (:require [ccc]))" (h/file-uri "file:///bbb.clj") components)
    (h/load-code "(ns ccc)" (h/file-uri "file:///ccc.clj") components)
    (is (= 0 (count (q/uri-dependents-analysis (h/db components) "file:///aaa.clj"))))
    (is (= 1 (count (q/uri-dependents-analysis (h/db components) "file:///bbb.clj"))))
    (is (= 2 (count (q/uri-dependents-analysis (h/db components) "file:///ccc.clj"))))))
...
(deftest uri-dependencies-analysis
  (let [components (h/make-components)]
    ...
    (h/load-code "(ns bbb (:require [ccc]))" (h/file-uri "file:///bbb.clj") components)
    (h/load-code "(ns ccc)" (h/file-uri "file:///ccc.clj") components)
    (is (= 2 (count (q/uri-dependencies-analysis (h/db components) "file:///aaa.clj"))))
    (is (= 1 (count (q/uri-dependencies-analysis (h/db components) "file:///bbb.clj"))))
    (is (= 0 (count (q/uri-dependencies-analysis (h/db components) "file:///ccc.clj"))))))
...
(deftest find-element-under-cursor
  (let [components (h/make-components)
        code (str "(ns a.b.c (:require [d.e.f :as |f-alias]))\n"
        ...
        [[alias-r alias-c]
         [param-r param-c]
         [x-r x-c]
         [unknown-r unknown-c]] (h/load-code code h/default-uri components)
        db (h/db components)]
    (h/assert-submap
      '{:alias f-alias}
...
(deftest find-references-from-cursor
  (let [components (h/make-components)
        a-code (h/code "(ns a.b.c (:require [d.e.f :as |f-alias]))"
        ...
        [[alias-r alias-c]
         ...
         [x-use-r x-use-c]
         [unknown-r unknown-c]] (h/load-code a-code h/default-uri components)
        [[a-foo-kw-r a-foo-kw-c]] (h/load-code "|:foo-kw" (h/file-uri "file:///b.clj") components)
        [[b-foo-kw-r b-foo-kw-c]
         [c-foo-kw-r c-foo-kw-c]
         [d-foo-kw-r d-foo-kw-c]] (h/load-code (h/code "|:foo-kw"
                                                       "(let [{:keys [|foo-kw]} {|:foo-kw 1}]"
                                                       "  foo-kw)") (h/file-uri "file:///c.clj") components)
        [[a-baz-kw-r a-baz-kw-c]
         [b-baz-kw-r b-baz-kw-c]] (h/load-code (h/code "(ns baz)"
                                                       "|::foo/foo-kw"
                                                       "|::baz-kw") (h/file-uri "file:///baz.clj") components)
        db (h/db components)]
    (h/assert-submaps
...
(deftest find-references-from-cursor-cljc
  (let [components (h/make-components)
        ...
        [[alias-r alias-c]
         ...
         [alias-use-r alias-use-c]
         [x-use-r x-use-c]
         [unknown-r unknown-c]] (h/load-code code (h/file-uri "file:///a.cljc") components)
        db (h/db components)]
    (h/assert-submaps
      [{:name 'x :name-row x-r :name-col x-c}
       {:name 'x :name-row x-use-r :name-col x-use-c}]
...
(deftest find-references-from-namespace-definition
  (let [components (h/make-components)
        [[ns-def-r ns-def-c]] (h/load-code (h/code "(ns |some.cool-ns) (def foo 1)") h/default-uri components)
        _ (h/load-code (h/code "(ns |other.cool-ns"
                               " (:require [some.cool-ns :as s])) s/foo")
                       (h/file-uri "file:///b.clj") components)
        _ (h/load-code (h/code "(ns |another.cool-ns) :some.cool-ns/bar")
    ...
    (testing "from ns definition"
      (h/assert-submaps
        common-references
        (q/find-references-from-cursor (h/db components) (h/file-path "/a.clj") ns-def-r ns-def-c false)))
    (testing "Including definition"
      (h/assert-submaps
        (concat [{:filename (h/file-path "/a.clj")
                ...
                common-references)
        (q/find-references-from-cursor (h/db components) (h/file-path "/a.clj") ns-def-r ns-def-c true)))))

(deftest find-references-from-namespace-usage
  (let [components (h/make-components)
        _ (h/load-code (h/code "(ns some.cool-ns) (def foo 1)") h/default-uri components)
        [[usage-r usage-c]] (h/load-code (h/code "(ns other.cool-ns"
                                                 " (:require [|some.cool-ns :as s])) s/foo")
                                         (h/file-uri "file:///b.clj") components)
        _ (h/load-code (h/code "(ns another.cool-ns) :some.cool-ns/bar")
                       (h/file-uri "file:///c.clj") components)]
      ...
      (h/assert-submaps
        [{:filename (h/file-path "/b.clj")
         ...
         {:filename (h/file-path "/c.clj")
          ...
          :name "bar"
          :ns 'some.cool-ns}]
        (q/find-references-from-cursor (h/db components) (h/file-path "/b.clj") usage-r usage-c false)))))

(deftest find-references-from-defrecord
  (let [components (h/make-components)
        code (str "(defrecord |MyRecord [])\n"
                  "(|MyRecord)"
                  "(|->MyRecord)"
                  "(|map->MyRecord)")
        [[def-r def-c]
         [raw-r raw-c]
         [to-r to-c]
         [map-to-r map-to-c]] (h/load-code code (h/file-uri "file:///a.clj") components)
        db (h/db components)]
    (h/assert-submaps
...
(deftest find-references-excluding-function-different-arity
  (let [components (h/make-components)
        ...
        b-code (h/code "(ns b"
                       ...
                       "  (|aa/bar))")
        [[bar-def-r bar-def-c]
         [bar-usa-r bar-usa-c]] (h/load-code a-code (h/file-uri "file:///a.clj") components)
        [[bar-usa-b-r bar-usa-b-c]] (h/load-code b-code (h/file-uri "file:///b.clj") components)
        db (h/db components)]
...
(deftest find-references-from-protocol-impl
  (let [components (h/make-components)]
    (h/load-code (h/code "(ns a)"
                         "(defprotocol Foo"
                         "  (something []))") h/default-uri components)
    (h/load-code (h/code "(ns b (:require [a :as f]))"
                         "(defrecord FooImpl1 []"
                         " f/Foo"
                         " (something [_] 123))"
                         "(defrecord FooImpl2 []"
                         ...
                         "(f/something (->FooImpl2))") (h/file-uri "file:///b.clj") components)
      ...
      (h/assert-submaps
        '[{:row 8 :col 1 :end-row 8 :end-col 27
          ...
          {:name-col 2 :name-row 9 :name-end-row 9 :name-end-col 13
           ...
           :bucket :var-usages
           :to a}]
        (q/find-references-from-cursor (h/db components) (h/file-path "/b.clj") 7 9 false)))))

(deftest find-references-for-defmulti
  (let [components (h/make-components)
        [[defmulti-r defmulti-c]]
        (h/load-code (h/code "(ns a)"
                             "(defmulti |my-multi :some-key)") h/default-uri components)
        ...
        (h/load-code (h/code "(ns b (:require [a :as f]))"
                             ...
                             " [_]"
                             " :foo)"
                             "(|f/my-multi {:some-value 123})") (h/file-uri "file:///b.clj") components)
    ...
    (testing "from defmulti method name"
      (h/assert-submaps
        references
        (q/find-references-from-cursor (h/db components) (h/file-path "/a.clj") defmulti-r defmulti-c false)))
    (testing "from defmethod method name"
      (h/assert-submaps
        references
        (q/find-references-from-cursor (h/db components) (h/file-path "/b.clj") defmethod-r defmethod-c false)))
...
(deftest find-references-for-defmulti-without-usages
  (let [components (h/make-components)
        ...
        (h/load-code (h/code "(ns b (:require [a :as f]))"
                             "(defmethod |f/my-multi :some-value"
                             " [_]"
                             " :foo)") (h/file-uri "file:///b.clj") components)
        references '[;; defmethod
                     {:name-row 2 :name-col 12 :name-end-row 2 :name-end-col 22
                      :row 2 :col 12 :end-row 2 :end-col 22
                      :name my-multi
...
(deftest find-references-from-declare
  (let [components (h/make-components)
        ...
        (h/load-code (h/code "(ns a)"
                             "(declare |my-declared)"
                             "(def |my-declared 1)"
                             "(inc |my-declared)") h/default-uri components)
    ...
    (testing "from declare"
      (h/assert-submaps
        [usage-element]
        (q/find-references-from-cursor (h/db components) (h/file-path "/a.clj") declare-r declare-c false)))
    (testing "from def"
      (h/assert-submaps
        [usage-element]
        (q/find-references-from-cursor (h/db components) (h/file-path "/a.clj") def-r def-c false)))
    (testing "from usage name"
      (h/assert-submaps
        [usage-element]
        (q/find-references-from-cursor (h/db components) (h/file-path "/a.clj") usage-r usage-c false)))))
...
(deftest find-references-from-declare-without-usages
  (let [components (h/make-components)
        ...
        (h/load-code (h/code "(ns a)"
                             "(declare |my-declared)"
                             "(def |my-declared 1)") h/default-uri components)]
    (testing "from declare"
      (h/assert-submaps
        '[]
        (q/find-references-from-cursor (h/db components) (h/file-path "/a.clj") declare-r declare-c false)))
    (testing "from def"
      (h/assert-submaps
        '[]
        (q/find-references-from-cursor (h/db components) (h/file-path "/b.clj") def-r def-c false)))))
...
(deftest find-definition-from-cursor
  (let [components (h/make-components)
        ...
        [[alias-r alias-c]
         ...
         [x-use-r x-use-c]
         [unknown-r unknown-c]] (h/load-code code h/default-uri components)
        _ (h/load-code "(ns d.e.f) (def foo 1)" (h/file-uri "file:///b.clj") components)
        db (h/db components)]
    (h/assert-submap
      {:name 'x :name-row x-r :name-col x-c}
...
(deftest find-definition-from-cursor-when-duplicate-from-external-analysis
  (let [components (h/make-components)
        _ (h/load-code (h/code "(ns foo) (def bar)") "jar:file:///some.jar!/some-jar.clj" components)
        _ (h/load-code (h/code "(ns foo) (def bar)") (h/file-uri "file:///a.clj") components)
        [[bar-r bar-c]] (h/load-code (h/code "(ns baz (:require [foo :as f]))"
                                             "|f/bar") (h/file-uri "file:///b.clj") components)
        db (h/db components)]
    (h/assert-submap
      {:name 'bar :filename (h/file-path "/a.clj")}
      (q/find-definition-from-cursor db (h/file-path "/b.clj") bar-r bar-c))))

(defn assert-find-definition-from-cursor-when-it-has-same-namespace-from-clj-and-cljs [components]
  (testing "when on a clj file"
    (let [[[bar-r bar-c]] (h/load-code (h/code "(ns baz (:require [foo :as f]))"
                                               "|f/bar") (h/file-uri "file:///b.clj") components)
          db (h/db components)]
      (h/assert-submap
  ...
  (testing "when on a cljs file"
    (let [[[bar-r bar-c]] (h/load-code (h/code "(ns baz (:require [foo :as f]))"
                                               "|f/bar") (h/file-uri "file:///b.cljs") components)
          db (h/db components)]
      (h/assert-submap
  ...
  (testing "when on a cljc file"
    (let [[[bar-r bar-c]] (h/load-code (h/code "(ns baz (:require [foo :as f]))"
                                               "|f/bar") (h/file-uri "file:///b.cljc") components)
          db (h/db components)]
      (h/assert-submap
        {:name 'bar :filename (h/file-path "/some.jar:some-jar.clj")}
  ...
  (testing "when on a cljc file with multiple langs available"
    (let [[[bar-r-clj bar-c-clj]
           [bar-r-cljs bar-c-cljs]] (h/load-code (h/code "(ns baz #?(:clj (:require [foo :as fc]) :cljs (:require [foo :as fs])))"
                                                         "|fc/bar"
                                                         "|fs/bar") (h/file-uri "file:///b.cljc") components)
          db (h/db components)]
      (h/assert-submap
        {:name 'bar :filename (h/file-path "/some.jar:some-jar.clj")}
  ...
  (testing "when a macro is require-macros on cljs, being the var-definition on a clj/cljc file."
    (h/load-code (h/code "(ns some.foo) (defmacro bar [& body] @body)") (h/file-uri "file:///some/foo.clj") components)
    (h/load-code (h/code "(ns some.foo (:require-macros [some.foo])) (def baz)") (h/file-uri "file:///some/foo.cljs") components)
    (let [[[bar-r bar-c]] (h/load-code (h/code "(ns a (:require [some.foo :as f])) (f/|bar 1)") (h/file-uri "file:///a.cljs") components)
          db (h/db components)]
      (h/assert-submap
...
(deftest find-definition-from-cursor-when-it-has-same-namespace-from-clj-and-cljs
  (let [components (h/make-components)]
    (h/load-code (h/code "(ns foo) (def bar)") (h/file-uri "jar:file:///some.jar!/some-jar.clj") components)
    (h/load-code (h/code "(ns foo) (def bar)") (h/file-uri "jar:file:///some.jar!/other-jar.cljs") components)
    (assert-find-definition-from-cursor-when-it-has-same-namespace-from-clj-and-cljs components)))
...
(deftest find-definition-from-cursor-when-declared
  (let [components (h/make-components)]
    (testing "declared, then defined"
      (let [[[usage-r usage-c]
             [defn-r _]] (h/load-code
                           (h/code "(ns foo)"
                                   "(declare bar)"
                                   "(|bar)"
                                   "(defn |bar [] 1)") (h/file-uri "file:///a.clj") components)
            db (h/db components)]
        (h/assert-submap
    ...
    (testing "only declared"
      (let [[[decl-r _]
             [usage-r usage-c]] (h/load-code
                                  (h/code "(ns foo)"
                                          "(declare |bar)"
                                          "(|bar)") (h/file-uri "file:///a.clj") components)
            db (h/db components)]
        (h/assert-submap
...
(deftest find-definition-from-namespace-alias
  (let [components (h/make-components)]
    (h/load-code (h/code "(ns foo.bar) (def a 1)") (h/file-uri "file:///a.clj") components)
    (let [[[foob-r foob-c]] (h/load-code (h/code "(ns foo.baz (:require [foo.bar :as |foob]))") (h/file-uri "file:///b.clj") components)
          db (h/db components)]
      (h/assert-submap
...
(deftest find-definition-from-cursor-when-on-potemkin
  (let [components (h/make-components)]
    (h/load-code (h/code "(ns foo.impl) (def bar)") (h/file-uri "file:///b.clj") components)
    (let [[[bar-r bar-c]] (h/load-code
                            (h/code "(ns foo.api"
                                    "  (:require [potemkin :refer [import-vars]]"
                                    "            [foo.impl :as impl]))"
                                    "(import-vars |impl/bar)") (h/file-uri "file:///a.clj") components)
          db (h/db components)]
      (h/assert-submap
...
#_(deftest find-definition-from-java-class-usage
    (let [components (h/make-components)]
      (h/load-code (h/code "package project;"
                           "class Foo {}")
                   (h/file-uri "file:///project/Foo.java") components)
      (h/load-code (h/code "123456") (h/file-uri "file:///project/Foo.class") components)
      (testing "Finding java source even if class exists"
        (let [[[foo-r foo-c]] (h/load-code (h/code "(ns a (:import (project Foo)))"
                                                   "(|Foo.)")
                                           (h/file-uri "file:///a.clj") components)
              db (h/db components)]
          (h/assert-submap
            {} ;; TODO
            (q/find-definition-from-cursor db (h/file-path "/a.clj") foo-r foo-c))))))

(deftest find-declaration-from-cursor
  (let [components (h/make-components)]
    (h/load-code (h/code "(ns colors) (def orange 123)") h/default-uri components)
    (let [[[something-r something-c]
           [orange-r orange-c]
           [other-r other-c]] (h/load-code
                                (h/code "(ns sample"
...
(deftest find-declaration-from-cursor-in-cljc
  (let [components (h/make-components)
        [[clj-ns-r clj-ns-c]
         ...
         [cljs-usage-r cljs-usage-c]] (h/load-code
                                        (h/code "(ns sample"
                                                "  (:require #?(:clj |foo"
                                                "               :cljs |foo)))"
                                                "#?(:clj foo/som|ething"
                                                "   :cljs foo/som|ething)")
                                        "file:///b.cljc" components)
        db (h/db components)]
...
(deftest find-implementations-from-cursor-protocols
  (let [components (h/make-components)]
    ...
    (h/load-code (h/code "(ns b (:require [a :as f]))"
                         ...
                         "(defn make-foo [] (reify f/Foo (something [_] 123)))") (h/file-uri "file:///b.clj") components)
      ...
      (h/assert-submaps
        '[{:name Foo
          ...
          {:name-row 10 :name-col 26 :name-end-row 10 :name-end-col 31
           ...
           :bucket :var-usages
           :to a}]
        (q/find-implementations-from-cursor (h/db components) (h/file-path "/a.clj") 2 16)))
      ...
      (h/assert-submaps
        [{:impl-ns 'b
         ...
         {:impl-ns 'b
          ...
          :filename "/b.clj"
          :bucket :protocol-impls}]
        (q/find-implementations-from-cursor (h/db components) (h/file-path "/a.clj") 3 4)))
      ...
      (h/assert-submaps
        [{:impl-ns 'b
         ...
         {:impl-ns 'b
          ...
          :filename "/b.clj"
          :bucket :protocol-impls}]
        (q/find-implementations-from-cursor (h/db components) (h/file-path "/b.clj") 9 2)))))

(deftest find-implementations-from-cursor-defmulti
  (let [components (h/make-components)]
    (h/load-code (h/code "(ns a)"
                         "(defmulti foo :some-key)") h/default-uri components)
    (h/load-code (h/code "(ns b (:require [a :as f]))"
                         ...
                         "(f/foo {})") (h/file-uri "file:///b.clj") components)
      ...
      (h/assert-submaps
        '[{:name foo
          ...
          {:name-row 5 :name-col 12 :name-end-row 5 :name-end-col 17
           ...
           :bucket :var-usages
           :to a}]
        (q/find-implementations-from-cursor (h/db components) (h/file-path "/a.clj") 2 12)))
      ...
      (h/assert-submaps
        '[{:name foo
          ...
          {:name-row 5 :name-col 12 :name-end-row 5 :name-end-col 17
           ...
           :bucket :var-usages
           :to a}]
        (q/find-implementations-from-cursor (h/db components) (h/file-path "/b.clj") 2 13)))
      ...
      (h/assert-submaps
        '[{:name foo
          ...
          {:name-row 5 :name-col 12 :name-end-row 5 :name-end-col 17
           ...
           :bucket :var-usages
           :to a}]
        (q/find-implementations-from-cursor (h/db components) (h/file-path "/b.clj") 8 2)))))

(deftest find-unused-aliases
  (let [components (h/make-components)]
    (testing "clj"
      (testing "used require via alias"
        (h/load-code "(ns a (:require [x :as f])) f/foo" h/default-uri components)
        (is (= '#{}
               (q/find-unused-aliases (h/db components) (h/file-path "/a.clj")))))
      (testing "used require via full-ns"
        (h/load-code "(ns a (:require [x :as f])) x/foo" h/default-uri components)
        (is (= '#{}
               (q/find-unused-aliases (h/db components) (h/file-path "/a.clj")))))
      (testing "full-ns require"
        (h/load-code "(ns a (:require [x] y)) foo" h/default-uri components)
        (is (= '#{}
               (q/find-unused-aliases (h/db components) (h/file-path "/a.clj")))))
      (testing "single unused-alias"
        (h/load-code "(ns a (:require [x :as f]))" h/default-uri components)
        (is (= '#{x}
               (q/find-unused-aliases (h/db components) (h/file-path "/a.clj")))))
      (testing "used and unused aliases"
        (h/load-code "(ns a (:require [x :as f] [foo] x [bar :as b] [y :refer [m]] [z :refer [o i]])) o" h/default-uri components)
        (is (= '#{y bar}
               (q/find-unused-aliases (h/db components) (h/file-path "/a.clj"))))))
    (testing "cljc"
      (testing "used require via alias"
        (h/load-code "(ns a (:require [x :as f])) f/foo" (h/file-uri "file:///a.cljc") components)
        (is (= '#{}
               (q/find-unused-aliases (h/db components) (h/file-path "/a.cljc")))))
      (testing "used require via full-ns"
        (h/load-code "(ns a (:require [x :as f])) x/foo" (h/file-uri "file:///a.cljc") components)
        (is (= '#{}
               (q/find-unused-aliases (h/db components) (h/file-path "/a.cljc")))))
      (testing "full-ns require"
        (h/load-code "(ns a (:require [x] y)) foo" (h/file-uri "file:///a.cljc") components)
        (is (= '#{}
               (q/find-unused-aliases (h/db components) (h/file-path "/a.cljc")))))
      (testing "single unused-alias"
        (h/load-code "(ns a (:require [x :as f]))" (h/file-uri "file:///a.cljc") components)
        (is (= '#{x}
               (q/find-unused-aliases (h/db components) (h/file-path "/a.cljc")))))
      (testing "used and unused aliases"
        (h/load-code "(ns a (:require [x :as f] [foo] x [bar :as b] [y :refer [m]] [z :refer [o i]])) o" (h/file-uri "file:///a.cljc") components)
        (is (= '#{y bar}
               (q/find-unused-aliases (h/db components) (h/file-path "/a.cljc")))))
      (testing "used alias in a reader conditional"
        (h/load-code "(ns a (:require [y :as o] [x :as f])) #?(:clj f/foo)" (h/file-uri "file:///a.cljc") components)
        (is (= '#{y}
               (q/find-unused-aliases (h/db components) (h/file-path "/a.cljc"))))))))
  ...
  (let [components (h/make-components)]
    (testing "clj"
      (testing "used require via refer"
        (h/load-code "(ns a (:require [x :refer [foo]])) foo" h/default-uri components)
        (is (= '#{}
               (q/find-unused-refers (h/db components) (h/file-path "/a.clj")))))
      (testing "multiple used refers"
        (h/load-code "(ns a (:require [x :refer [foo bar baz]])) foo bar baz" h/default-uri components)
        (is (= '#{}
               (q/find-unused-refers (h/db components) (h/file-path "/a.clj")))))
      (testing "single unused refer"
        (h/load-code "(ns a (:require [x :refer [foo]]))" h/default-uri components)
        (is (= '#{x/foo}
               (q/find-unused-refers (h/db components) (h/file-path "/a.clj")))))
      (testing "multiple unused refer"
        (h/load-code "(ns a (:require [x :refer [foo bar]]))" h/default-uri components)
        (is (= '#{x/foo x/bar}
               (q/find-unused-refers (h/db components) (h/file-path "/a.clj")))))
      (testing "multiple unused refer and used"
        (h/load-code "(ns a (:require [x :refer [foo bar baz]])) bar" h/default-uri components)
        (is (= '#{x/foo x/baz}
               (q/find-unused-refers (h/db components) (h/file-path "/a.clj"))))))
    (testing "cljc"
      (testing "used require via refer"
        (h/load-code "(ns a (:require [x :refer [foo]])) foo" (h/file-uri "file:///a.cljc") components)
        (is (= '#{}
               (q/find-unused-refers (h/db components) (h/file-path "/a.cljc")))))
      (testing "multiple used refers"
        (h/load-code "(ns a (:require [x :refer [foo bar baz]])) foo bar baz" (h/file-uri "file:///a.cljc") components)
        (is (= '#{}
               (q/find-unused-refers (h/db components) (h/file-path "/a.cljc")))))
      (testing "single unused refer"
        (h/load-code "(ns a (:require [x :refer [foo]]))" (h/file-uri "file:///a.cljc") components)
        (is (= '#{x/foo}
               (q/find-unused-refers (h/db components) (h/file-path "/a.cljc")))))
      (testing "multiple unused refer"
        (h/load-code "(ns a (:require [x :refer [foo bar]]))" (h/file-uri "file:///a.cljc") components)
        (is (= '#{x/foo x/bar}
               (q/find-unused-refers (h/db components) (h/file-path "/a.cljc")))))
      (testing "multiple unused refer and used"
        (h/load-code "(ns a (:require [x :refer [foo bar baz]])) bar" (h/file-uri "file:///a.cljc") components)
        (is (= '#{x/foo x/baz}
               (q/find-unused-refers (h/db components) (h/file-path "/a.cljc")))))
      (testing "used refer in a reader conditional"
        (h/load-code "(ns a (:require [y :refer [o]] [x :refer [f]])) #?(:clj f)" (h/file-uri "file:///a.cljc") components)
        (is (= '#{y/o}
               (q/find-unused-refers (h/db components) (h/file-path "/a.cljc"))))))))
  ...
  (let [components (h/make-components)]
    (testing "clj"
      (testing "single used full import"
        (h/load-code "(ns a (:import java.util.Date)) Date." h/default-uri components)
        (is (= '#{}
               (q/find-unused-imports (h/db components) (h/file-path "/a.clj")))))
      (testing "single unused full import"
        (h/load-code "(ns a (:import java.util.Date))" h/default-uri components)
        (is (= '#{java.util.Date}
               (q/find-unused-imports (h/db components) (h/file-path "/a.clj")))))
      (testing "multiple unused full imports"
        (h/load-code "(ns a (:import java.util.Date java.util.Calendar java.time.LocalDateTime))" h/default-uri components)
        (is (= '#{java.util.Date java.util.Calendar java.time.LocalDateTime}
               (q/find-unused-imports (h/db components) (h/file-path "/a.clj")))))
      (testing "multiple unused package imports"
        (h/load-code "(ns a (:import [java.util Date Calendar] [java.time LocalDateTime]))" h/default-uri components)
        (is (= '#{java.util.Date java.util.Calendar java.time.LocalDateTime}
               (q/find-unused-imports (h/db components) (h/file-path "/a.clj")))))
      (testing "multiple unused and used imports"
        (h/load-code "(ns a (:import [java.util Date Calendar] [java.time LocalTime LocalDateTime])) LocalTime." h/default-uri components)
        (is (= '#{java.util.Date java.util.Calendar java.time.LocalDateTime}
               (q/find-unused-imports (h/db components) (h/file-path "/a.clj"))))))
    (testing "cljc"
      (testing "single used full import"
        (h/load-code "(ns a (:import java.util.Date)) Date." (h/file-uri "file:///a.cljc") components)
        (is (= '#{}
               (q/find-unused-imports (h/db components) (h/file-path "/a.cljc")))))
      (testing "single unused full import"
        (h/load-code "(ns a (:import java.util.Date))" (h/file-uri "file:///a.cljc") components)
        (is (= '#{java.util.Date}
               (q/find-unused-imports (h/db components) (h/file-path "/a.cljc")))))
      (testing "multiple unused full imports"
        (h/load-code "(ns a (:import java.util.Date java.util.Calendar java.time.LocalDateTime))" (h/file-uri "file:///a.cljc") components)
        (is (= '#{java.util.Date java.util.Calendar java.time.LocalDateTime}
               (q/find-unused-imports (h/db components) (h/file-path "/a.cljc")))))
      (testing "multiple unused package imports"
        (h/load-code "(ns a (:import [java.util Date Calendar] [java.time LocalDateTime]))" (h/file-uri "file:///a.cljc") components)
        (is (= '#{java.util.Date java.util.Calendar java.time.LocalDateTime}
               (q/find-unused-imports (h/db components) (h/file-path "/a.cljc")))))
      (testing "multiple unused and used imports"
        (h/load-code "(ns a (:import [java.util Date Calendar] [java.time LocalTime LocalDateTime])) LocalTime." (h/file-uri "file:///a.cljc") components)
        (is (= '#{java.util.Date java.util.Calendar java.time.LocalDateTime}
               (q/find-unused-imports (h/db components) (h/file-path "/a.cljc")))))
      (testing "used import in a reader conditional"
        (h/load-code "(ns a (:import [java.util Date Calendar] [java.time LocalTime LocalDateTime])) #?(:clj LocalTime.)" (h/file-uri "file:///a.cljc") components)
        (is (= '#{java.util.Date java.util.Calendar java.time.LocalDateTime}
               (q/find-unused-imports (h/db components) (h/file-path "/a.cljc"))))))))
  ...
  (let [components (h/make-components)]
    (testing "inside let"
      (let [[[sum-pos-r sum-pos-c]
            ...
            (h/load-code "(ns a) (let [a 2 b 1] |(+ 2 b)| (- 2 a))" h/default-uri components)]
        (h/assert-submaps
          [{:name 'b}]
          (q/find-local-usages-under-form (h/db components) (h/file-path "/a.clj")
                                          {:row sum-pos-r, :col sum-pos-c
                                           :end-row sum-end-pos-r, :end-col sum-end-pos-c}))))
    (testing "inside defn"
      (let [[[let-pos-r let-pos-c]
             [let-end-pos-r let-end-pos-c]]
            (h/load-code "(ns a) (defn ab [b] |(let [a 1] (b a))|) (defn other [c] c)" h/default-uri components)]
        (h/assert-submaps
          [{:name 'b}]
          (q/find-local-usages-under-form (h/db components) (h/file-path "/a.clj")
