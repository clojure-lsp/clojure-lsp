...
(defmethod refactor :add-missing-libspec [{:keys [loc] {:keys [db]} :components}]
...
(defmethod refactor :resolve-macro-as [{:keys [loc uri] {:keys [db logger]} :components}]
  (f.resolve-macro/resolve-macro-as! loc uri db logger))
